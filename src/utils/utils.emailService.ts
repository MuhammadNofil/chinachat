...
export class EmailService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.from = `China Chat <${this.configService.get('EMAIL_FROM')}>`;
  ...
  async sendWelcome(user: EmailUser, url: string, payload) {
  ...
  async forgotPassword(user: EmailUser, url: string, payload) {
    await this.send(user, 'forgotPassword', payload.subject, '', payload);
  }

  async confirmSignup(user: EmailUser, url: string, payload) {
    await this.send(user, 'confirmSignup', payload.subject, '', payload);
  }
}
